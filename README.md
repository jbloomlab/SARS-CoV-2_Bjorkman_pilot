...
Design and experiments by Alex Cohen of Pamela Bjorkman's lab, with analysis by Allie Greaney and [Jesse Bloom](https://research.fhcrc.org/bloom/en.html).
...
First, set up the computing environment, which is partially done via `conda`. I prefer to use [`mamba`](https://anaconda.org/conda-forge/mamba), which is basically just a faster version of `conda`.
Ensure you have `conda` and `mamba` installed; if not install it via Miniconda as described [here](https://docs.conda.io/projects/conda/en/latest/user-guide/install/#regular-installation).
